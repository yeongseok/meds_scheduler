...
interface GuardiansPageProps {
  onNavigateToPermissionRequests?: () => void;
}

export function GuardiansPage({ onNavigateToPermissionRequests }: GuardiansPageProps = {}) {
  ...
  // Fetch received invitations with real-time updates
  const {
    invitations: receivedInvitations,
    loading: receivedLoading,
    acceptInvitation,
    declineInvitation
  } = useReceivedInvitations(user?.email || undefined, true);
  
  // Fetch sent invitations with real-time updates
  const {
    invitations: sentInvitations,
    loading: sentLoading,
    cancelInvitation,
    resendInvitation
  } = useSentInvitations(user?.uid, true);
  ...
  const handleUpdatePermissions = async () => {
    ...
    try {
      await updateGuardian(editingGuardian.id, {
        ...
        permissions: {
          viewMedications: true, // Always true
          viewHistory: editCanViewHistory,
          receiveAlerts: editCanGetNotifications
        }
      });
  ...
  const handleRemoveGuardian = async () => {
    ...
    try {
      await deleteGuardian(selectedGuardian.id);
      toast.success(
        language === 'ko' 
          ? `${selectedGuardian.name}님이 제거되었습니다` 
          : `${selectedGuardian.name} has been removed`,
        {
          className: 'bg-[#E8F1FC] border-[#3674B5] text-gray-800'
        }
      );
  ...
  const handleResendInvite = async () => {
    ...
    try {
      await resendInvitation(selectedInvite.id);
      toast.success(
        language === 'ko' 
  ...
  const handleCancelInvite = async () => {
    ...
    try {
      await cancelInvitation(selectedInvite.id);
  ...
  const handleAcceptInvitation = async () => {
    if (!selectedReceivedInvite || !user || !acceptRelationship) {
    ...
    try {
      await acceptInvitation(selectedReceivedInvite.id, user.uid, acceptRelationship);
      toast.success(
        language === 'ko' 
      ...
      toast.error(
        language === 'ko'
          ? `수락 실패: ${error.message}`
          : `Failed to accept: ${error.message}`,
        {
          className: 'bg-[#E8F1FC] border-[#3674B5] text-gray-800'
        }
      );
  ...
  const handleDeclineInvitation = async () => {
    if (!selectedReceivedInvite) return;
    
    try {
      await declineInvitation(selectedReceivedInvite.id);
      ...
      toast.error(
        language === 'ko'
          ? `거절 실패: ${error.message}`
          : `Failed to decline: ${error.message}`
      );
    }
  };

  const isLoading = authLoading || guardiansLoading || receivedLoading || sentLoading;
  ...
  return (
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-4">
        ...
        {onNavigateToPermissionRequests && (
          <div className="relative">
            <button
              onClick={onNavigateToPermissionRequests}
              className="w-full bg-[#E8F1FC] border-2 border-[#3674B5]/20 rounded-2xl p-4 flex items-center justify-between hover:border-[#3674B5]/40 transition-colors"
            >
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-[#3674B5] flex items-center justify-center">
                ...
                <div className="text-left">
                  ...
                  <div className="text-[16px] text-gray-600">
                    {language === 'ko' ? '보호자의 약 추가 요청 확인' : 'Review medicine requests'}
                  </div>
                </div>
              </div>
            </button>
            {/* Mock badge - would be dynamic in production */}
            <div className="absolute top-2 right-2 min-w-[20px] h-5 px-1.5 bg-[#FF4444] rounded-full flex items-center justify-center shadow-md">
              <span className="text-white text-[14px] leading-none">2</span>
            </div>
          </div>
        )}
        ...
        {!isLoading && receivedInvitations.length > 0 && (
          <div>
            <GuardianSectionHeader
              ...
              badgeColor="bg-brand-surface text-brand-accent"
            />
              ...
              {receivedInvitations.map((invite) => (
                <ReceivedInvitationCard
                  ...
                  invite={{
                    ...
                    receivedDate: new Date(invite.invitedAt).toLocaleDateString(),
                    initials: invite.fromUserName.split(' ').map(n => n[0]).join('').toUpperCase(),
                    color: 'bg-[#3674B5]'
                  }}
                  language={language}
                  onAccept={() => {
                    // Check if user already has 1 or more guardians in their care circle (free tier limit)
                    if (guardians.length >= 1 && !user?.isPro) {
                      setShowUpgradeDialog(true);
                    } else {
                      setSelectedReceivedInvite({ id: invite.id, fromName: invite.fromUserName });
            ...
            {guardians.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-[16px]">
                  {language === 'ko' ? '활성 보호자가 없습니다' : 'No active guardians'}
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                {guardians.map((guardian) => (
                  <ActiveGuardianCard
                    ...
                    guardian={{
                      ...
                      canViewHistory: guardian.permissions.viewHistory,
                      canGetNotifications: guardian.permissions.receiveAlerts,
                      addedDate: new Date(guardian.createdAt).toLocaleDateString(),
                      initials: guardian.guardianName.split(' ').map(n => n[0]).join('').toUpperCase(),
                      color: 'bg-[#3674B5]'
                    }}
                    language={language}
                    onEditPermissions={() => {
                      setEditingGuardian(guardian);
                      setEditCanViewHistory(guardian.permissions.viewHistory);
                      setEditCanGetNotifications(guardian.permissions.receiveAlerts);
                      setEditRelationship(guardian.relationship.toLowerCase());
                      setEditPermissionsOpen(true);
                    }}
                    onRemove={() => {
                      setSelectedGuardian({ id: guardian.id, name: guardian.guardianName });
              ...
              {sentInvitations.map((invite) => (
                <PendingInviteCard
                  ...
                  invite={{
                    ...
                    sentDate: new Date(invite.invitedAt).toLocaleDateString(),
                    status: 'pending'
                  }}
                  language={language}
                  onResend={() => {
                    setSelectedInvite({ id: invite.id, email: invite.toEmail });
        ...
        <DialogContent className="bg-white">
          <DialogHeader>
            ...
            <DialogDescription className="text-gray-600 text-[16px]">
              {language === 'ko'
                ? `${editingGuardian?.guardianName}님이 케어 서클에서 액세스할 수 있는 항목을 관리하세요.`
                : `Manage what ${editingGuardian?.guardianName} can access in your Care Circle.`}
            </DialogDescription>
          </DialogHeader>
          
          <div className="space-y-4 mt-4">
            <div className="flex items-start space-x-3 p-3 bg-brand-surface rounded-lg border border-brand-primary">
              <Avatar className="w-10 h-10">
                <AvatarFallback className="bg-[#3674B5] text-white">
                  {editingGuardian?.guardianName.split(' ').map((n: string) => n[0]).join('').toUpperCase()}
                </AvatarFallback>
              </Avatar>
              <div className="flex-1">
                <h4 className="font-semibold text-gray-800 text-[16px]">{editingGuardian?.guardianName}</h4>
                <p className="text-xs text-gray-600 text-[14px]">{editingGuardian?.guardianEmail}</p>
          ...
          <div className="flex gap-3 mt-4">
            ...
            <Button
              ...
              className="flex-1 bg-brand-accent hover:bg-brand-accent/90 text-[16px]"
            >
              {language === 'ko' ? '저장' : 'Save'}
            </Button>
          </div>
      ...
      <Dialog open={showUpgradeDialog} onOpenChange={setShowUpgradeDialog}>
        <DialogContent className="bg-white max-w-md rounded-3xl">
          ...
          <div className="py-6 space-y-4">
            <div className="bg-brand-surface p-4 rounded-xl border-2 border-brand-primary">
          ...
          <div className="flex flex-col gap-3">
            <Button 
              className="w-full bg-brand-accent hover:bg-brand-accent/90 text-white text-[16px] h-12"
              onClick={() => {
