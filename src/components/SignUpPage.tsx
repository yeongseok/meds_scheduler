...
import { Mail, Lock, Eye, EyeOff, User, Smartphone, ArrowLeft, AlertCircle } from 'lucide-react';
...
export function SignUpPage({ onBackToLogin, onSignUp, onNavigateToTerms, onNavigateToPrivacy, onNavigateToEmailSignUp, onNavigateToPhoneSignUp }: SignUpPageProps) {
    ...
    <div className="h-screen flex flex-col items-center justify-center bg-white">
      ...
      <div className="w-full max-w-md px-6 z-10 py-8 overflow-y-auto h-screen">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            {language === 'ko' ? '약드세요에 가입하여 약을 관리하세요' : 'Join MediRemind to manage your medications'}
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-start space-x-3 pt-2">
              ...
              >
                {language === 'ko' ? (
                  <>
                    <button type="button" onClick={onNavigateToTerms} className="text-[#2563A8] hover:text-[#3674B5] hover:underline">
                      이용약관
                    </button>
                    {' '}및{' '}
                    <button type="button" onClick={onNavigateToPrivacy} className="text-[#2563A8] hover:text-[#3674B5] hover:underline">
                      개인정보 처리방침
                    </button>
                    에 동의합니다
                  </>
        ...
        <div className="mb-6">
          ...
          <div className="mt-6 grid grid-cols-2 gap-3">
            {/* Kakao */}
            <button type="button" className="h-11 rounded-xl bg-[#FEE500] hover:bg-[#FFD900] border-[#FEE500] hover:border-[#FFD900] flex items-center justify-center gap-2">
            ...
            {/* Google */}
            <button type="button" className="h-11 rounded-xl bg-white hover:bg-gray-50 border border-gray-200 flex items-center justify-center gap-2">
            ...
            <button 
              ...
              className="h-11 rounded-xl bg-white hover:bg-gray-50 border border-gray-200 flex items-center justify-center gap-2"
            >
              <Mail size={20} className="text-[#3674B5]" />
              <span className="text-sm text-gray-700">
            ...
            <button 
              ...
              className="h-11 rounded-xl bg-white hover:bg-gray-50 border border-gray-200 flex items-center justify-center gap-2"
            >
              <Smartphone size={20} className="text-[#3674B5]" />
              <span className="text-sm text-gray-700">
      ...
      <AlertDialog open={showTermsDialog} onOpenChange={setShowTermsDialog}>
        <AlertDialogContent className="max-w-sm mx-4 rounded-2xl bg-white border-gray-200 shadow-lg">
          <AlertDialogHeader>
            <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-[#3674B5] flex items-center justify-center">
            ...
            <AlertDialogDescription className="text-center">
              {language === 'ko' 
                ? '계정을 만들려면 이용약관에 동의해 주세요. 약드세요에 가입하기 전에 정책을 확인해 주시기 바랍니다.'
                : 'Please agree to the terms and conditions to create an account. Review our policies before joining MediRemind.'}
            </AlertDialogDescription>
          </AlertDialogHeader>
